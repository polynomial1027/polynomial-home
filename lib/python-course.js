...
const { englishCourse } = require('./python-course-en');
...
const chapters = [
  {
    ...
    lessons: [
      L('names-references-memory', '前置课：名称、引用与内存身份', 38, '先建立“名称绑定对象”的模型，再理解 id()、is、缓存和可变性带来的反直觉现象。', [
        H('变量不是盒子，而是绑定到对象的名称', 'Python 变量更准确的说法是名称。执行赋值时，名称会绑定到一个对象；把一个名称赋给另一个名称，通常会让两个名称暂时引用同一个对象，而不是立刻复制对象。'),
        C('score = 256\nalias = score\nprint(score, alias)\nprint(score is alias)'),
        H('对象的身份、类型和值', '每个对象都有身份、类型和值。type() 查看类型，id() 返回对象在本次运行期间的身份整数。在 CPython 中，这个整数通常就是对象的内存地址，因此可用 hex() 观察十六进制形式；其他 Python 实现不保证它是实际内存地址。'),
        C('value = [1, 2]\nprint(type(value))\nprint(id(value))\nprint(hex(id(value)))'),
        H('== 比较值，is 比较身份', '== 询问两个对象的值是否相等，is 询问两边是否为同一个对象。业务数据通常使用 ==；is 主要用于 None 或自己创建的哨兵对象等身份判断。不要用 is 比较普通数字或字符串的值。'),
        C('first = [1, 2]\nsecond = [1, 2]\nalias = first\nprint(first == second, first is second)\nprint(first == alias, first is alias)\nmissing = None\nprint(missing is None)'),
        H('CPython 的小整数缓存', '当前 CPython 会预先保存 -5 到 256（含边界）的小整数对象，所以分别构造的 256 可能拥有相同身份；257 通常不会。它是 CPython 实现细节，不是 Python 语言对所有解释器的保证，程序正确性绝不能依赖这个范围。'),
        C('cached_a = int("256")\ncached_b = int("256")\nother_a = int("257")\nother_b = int("257")\nprint(cached_a == cached_b, cached_a is cached_b)\nprint(other_a == other_b, other_a is other_b)'),
        H('为什么直接写 257 也可能得到同一个对象', '编译器可能在同一个代码单元中复用相同常量，字符串也可能被驻留。因此直接书写两个 257 时，is 的结果可能与分别在交互环境或运行时构造对象时不同。只能把这种现象当作实现优化来观察，不能当作数值比较规则。'),
        C('def compare_literals():\n    first = 257\n    second = 257\n    return first == second, first is second\n\nprint(compare_literals())\nprint(int("257") == int("257"))\nprint(int("257") is int("257"))'),
        H('不可变对象：操作后通常重新绑定', '整数、浮点数、字符串和元组是不可变对象。a += 1 不能修改原来的整数，而是得到另一个整数对象，再让 a 绑定到新对象；仍指向旧对象的 b 不受影响。'),
        C('a = 256\nb = a\nold_identity = id(a)\na += 1\nprint(a, b)\nprint(id(a) == old_identity)\nprint(b == 256)'),
        H('可变对象：多个名称可能共享修改', '列表、字典和集合是可变对象。若两个名称引用同一列表，通过任一名称修改列表，另一个名称都会观察到变化。需要互不影响的数据时，应按需要使用 copy()、切片或深拷贝。'),
        C('first = ["read"]\nalias = first\nalias.append("code")\nprint(first, alias, first is alias)\n\nindependent = first.copy()\nindependent.append("test")\nprint(first)\nprint(independent)'),
        H('函数参数采用对象共享模型', '调用函数时，形参会绑定到实参所引用的对象，这常称为对象共享传递（call by sharing）。函数内重新绑定形参不会改掉外部名称；通过形参修改可变对象，则调用者能够看到修改。它不等同于某些语言中可直接替换调用者变量的“按引用传递”。'),
        C('def increase(number):\n    number += 1\n    return number\n\ndef append_item(items):\n    items.append("new")\n\ncount = 256\nvalues = ["old"]\nprint(increase(count), count)\nappend_item(values)\nprint(values)'),
        H('字符串驻留与显式 intern', '解释器可能自动复用一部分字符串对象，但具体范围并不稳定。sys.intern() 可以显式驻留字符串，使相同文本共享身份；正常文本判断仍应使用 ==，不要把自动驻留当成程序逻辑。'),
        C('import sys\nleft = sys.intern("python_course")\nright = sys.intern("_".join(["python", "course"]))\nprint(left == right)\nprint(left is right)'),
        H('id() 只在对象存活期间唯一', 'id() 保证对象存活期间身份不变且不与另一个仍存活的对象冲突。对象销毁后，解释器可以把原来的身份整数重新用于新对象，所以不能把 id() 当作永久编号、数据库主键或跨进程标识。'),
        C('item = []\nidentity = id(item)\nprint(identity, hex(identity))\ndel item\nprint("The old identity no longer names a live object here.")'),
        H('把实现细节变成可靠的实践规则', '比较值时使用 ==；判断 None 或明确的哨兵时使用 is；共享可变对象前先考虑副作用，需要独立副本时显式复制；把 id() 用于学习和调试，不用于业务身份；不要依赖小整数缓存、常量复用或字符串驻留来决定程序结果。'),
...
const assignments = {
  ...
  'profile-line': {
    ...
    makeCases: locale => randomNames(5).map(name => { const age = crypto.randomInt(8, 81), chinese = `${name}今年${age}岁。`, english = `${name} is ${age} years old.`; return { args: [name, age], expected: locale === 'en' ? english : chinese, accepted: [chinese, english] }; })
  },
  'clean-name': {
    ...
    makeCases: () => ['  ada lovelace ', '\tgrace hopper\n', '  guido VAN rossum  ', 'lin'].map(value => ({ args: [value], expected: value.trim().split(' ').map(part => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(' ') }))
  },
  'sorted-copy': {
...
function publicCourse(locale = 'zh') {
  const course = {
  ...
  return locale === 'en' ? englishCourse(course) : course;
}

function getAssignment(id) { return assignments[id] || null; }
function makeEvaluation(assignment, locale = 'zh') { return { className: 'Solution', functionName: assignment.functionName, preserveArgs: Boolean(assignment.preserveArgs), language: locale, cases: assignment.makeCases(locale) }; }
function makeVisibleEvaluation(assignment, locale = 'zh') { const evaluation = makeEvaluation(assignment, locale); evaluation.cases = evaluation.cases.slice(0, 3); evaluation.revealCaseResults = true; return evaluation; }
