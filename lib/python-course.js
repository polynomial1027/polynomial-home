...
const assignments = {
  'hello-function': {
    ...
    prompt: '编写 hello_world(name)，返回格式完全为 Hello, 名字! 的字符串。不要在函数中写死测试名字。',
    functionName: 'hello_world', starterCode: 'def hello_world(name):\n    # 在这里返回欢迎语\n    pass\n',
  ...
  'profile-line': {
    ...
    prompt: '编写 profile_line(name, age)，返回“名字今年age岁。”。age 会是随机整数，必须正确转换并拼接。',
    functionName: 'profile_line', starterCode: 'def profile_line(name, age):\n    # 可以使用 f-string\n    pass\n',
  ...
  'clean-name': {
    ...
    prompt: '编写 clean_name(raw_name)，删除两端空白并把每个单词的首字母改为大写。',
    functionName: 'clean_name', starterCode: 'def clean_name(raw_name):\n    pass\n',
  ...
  'sorted-copy': {
    ...
    prompt: '编写 sorted_copy(names)，返回按字母升序排列的新列表，并且不能修改传入的原列表。',
    functionName: 'sorted_copy', preserveArgs: true, starterCode: 'def sorted_copy(names):\n    pass\n',
  ...
  'number-window': {
    ...
    prompt: '编写 number_window(start, stop)，返回从 start 开始、到 stop 之前结束的整数列表。',
    functionName: 'number_window', starterCode: 'def number_window(start, stop):\n    pass\n',
  ...
  'square-window': {
    ...
    prompt: '编写 square_window(start, stop)，返回区间内每个整数平方组成的列表。建议使用列表解析。',
    functionName: 'square_window', starterCode: 'def square_window(start, stop):\n    pass\n',
  ...
  'rectangle-area': {
    ...
    prompt: '编写 rectangle_area(dimensions)，dimensions 是包含宽和高的元组，返回面积。',
    functionName: 'rectangle_area', starterCode: 'def rectangle_area(dimensions):\n    pass\n',
...
function publicAssignment(assignment) {
  ...
  return { id: assignment.id, title: assignment.title, prompt: assignment.prompt, functionName: assignment.functionName, starterCode: assignment.starterCode };
}

function getAssignment(id) { return assignments[id] || null; }

function makeEvaluation(assignment) {
  return { functionName: assignment.functionName, preserveArgs: Boolean(assignment.preserveArgs), cases: assignment.makeCases() };
}

module.exports = { publicCourse, getAssignment, makeEvaluation };
