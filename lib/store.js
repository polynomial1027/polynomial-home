...
function initialState() {
  ...
  const state = {
    ...
    announcements: [{ id: 'learning-layout-v012', title: '0.1.2 · 网络学习课程入口与三栏布局更新', content: '网络学习首页新增课程选择卡片，目前可进入 Python 基础课程；课程页面恢复左侧目录、中间知识正文与作业、右侧代码实验区的布局，并针对中等屏幕和手机端提供自适应排列。0.1.2 作为完整功能首次整合后的新版本基线，后续系统更新公告标题将统一带版本号。', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { id: 'draft-navigation-admin-v1', title: '作业草稿、应用导航与公告管理更新', content: '网络学习新增“我的作业草稿”，可集中查看、继续编辑以及导出单份 Python 文件或全部 JSON。顶部导航将项目、小游戏、网盘、学习和聊天收纳到“应用”，更新公告移至其右侧；管理员公告管理默认折叠，并支持搜索标题和正文。', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { id: 'navigation-v1', title: '全站导航与页面标签统一', content: '统一首页、更新公告、项目、小游戏、开发者聊天和管理后台的导航顺序、名称与当前页面高亮；新增面包屑导航，并补齐游戏内部返回入口。', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { id: 'game-match3-v1', title: '01号小游戏：星轨消消乐上线', content: '新增完整消消乐游戏，包含关卡目标、步数限制、连锁计分、提示、暂停、新游戏以及按账号自动保存和读取进度。', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { id: 'update-v6', title: '聊天、游戏存档与管理功能更新', content: '新增账号游戏存档接口、更新公告、聊天记录分页管理、敏感词屏蔽，并扩展服务器参数设置。', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }],
    gameSaves: []
