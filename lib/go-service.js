...
class GoService {
  ...
  appendMove(game, color, data, actorId = null) {
    ...
    if (next.consecutivePasses >= 2) {
      game.status = 'scoring';
      game.scoring = { source: 'katago', pending: true, startedAt: move.at, error: null };
  ...
  async finalizeWithKataGo(game) {
    if (game.status !== 'scoring' || game.scoring?.thinking) return game;
    ...
    try {
      ...
      game.scoring = { ...game.scoring, pending: false, thinking: false, assessment: detail, error: null };
      game.engineError = null;
      this.finish(game, result, 'katago-score', winner, detail);
    } catch (error) {
