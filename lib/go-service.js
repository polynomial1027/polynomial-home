...
const {
  GoRuleError, applyMove, gameToSgf, getGroup, indexOf, normalizeDead, opposite,
  replay, scorePosition, sgfToStudy, standardHandicap
} = require('./go-engine');
...
class GoService {
  ...
  gameView(game, viewer) {
    ...
    return {
      ...
      scoring: game.scoring || null, result: game.result || null, resultDetail: game.resultDetail || null,
  ...
  lobby(user) {
    ...
    return {
      ...
      invitations: capabilities.multiplayer ? this.state.goInvitations.filter(item => item.fromUserId === user.id || item.toUserId === user.id).slice(-50).reverse().map(item => this.invitationView(item)) : [],
  ...
  appendMove(game, color, data, actorId = null) {
    ...
    if (next.consecutivePasses >= 2) {
      game.status = 'scoring'; game.scoring = { proposals: {}, confirmations: {}, dispute: false, startedAt: move.at };
  ...
  engineTurn(game) {
    ...
    const profile = (this.settings().goAiProfiles || []).find(item => item.id === game.config.aiProfileId) || { maxVisits: 50, maxTime: 2 };
    ...
    return this.kataGo.generateMove({
      ...
      maxVisits: profile.maxVisits, maxTime: profile.maxTime
    }).then(move => {
      game.engineThinking = false;
      if (move.resign) this.finish(game, `${opposite(engineColor)}+R`, 'resign', opposite(engineColor));
      else this.appendMove(game, engineColor, move, 'katago');
      this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
      return game;
    }).catch(error => {
      game.engineThinking = false;
  ...
  resign(user, id) {
    ...
    this.finish(game, `${opposite(color)}+R`, 'resign', opposite(color));
    this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
    return game;
  }
  ...
  undoAi(user, game) {
    ...
    game.status = 'active'; game.scoring = null; game.revision++; game.updatedAt = at; this.startTurn(game, this.position(game).toPlay);
    this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
    return game;
  }

  updateScoring(user, id, dead) {
    const game = this.gameFor(id, user);
    if (game.status !== 'scoring' || !this.isParticipant(game, user)) throw new GoRuleError('当前不在数目阶段', 'INVALID_STATE');
    const position = this.position(game), selected = normalizeDead(dead, position.size), points = new Set();
    for (const [x, y] of selected) {
      if (!position.board[indexOf(position.size, x, y)]) continue;
      getGroup(position.board, position.size, x, y).stones.forEach(([gx, gy]) => points.add(`${gx},${gy}`));
    }
    game.scoring.proposals[user.id] = [...points].sort();
    delete game.scoring.confirmations[user.id]; game.scoring.dispute = false; game.revision++; game.updatedAt = nowIso();
    this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
    return game;
  }

  confirmScoring(user, id) {
    const game = this.gameFor(id, user);
    if (game.status !== 'scoring' || !this.isParticipant(game, user)) throw new GoRuleError('当前不在数目阶段', 'INVALID_STATE');
    game.scoring.proposals[user.id] ||= [];
    game.scoring.confirmations[user.id] = nowIso();
    const ids = game.participantIds, confirmed = ids.every(userId => game.scoring.confirmations[userId]);
    if (confirmed) {
      const proposals = ids.map(userId => JSON.stringify(game.scoring.proposals[userId] || []));
      if (new Set(proposals).size === 1) {
        const position = this.position(game), result = scorePosition(position, { rules: game.config.rules, komi: game.config.komi, dead: game.scoring.proposals[ids[0]] });
        this.finish(game, result.result, 'score', result.winner, result);
      } else game.scoring.dispute = true;
    }
    game.revision++; game.updatedAt = nowIso();
    this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
    return game;
  ...
  acceptOpponentScore(user, id) {
    const game = this.gameFor(id, user), opponent = game.participantIds.find(item => item !== user.id);
    if (game.status !== 'scoring' || !opponent) throw new GoRuleError('当前不能接受对方数目方案', 'INVALID_STATE');
    if (!Object.prototype.hasOwnProperty.call(game.scoring.proposals, opponent)) throw new GoRuleError('对方尚未提交数目方案', 'INVALID_STATE');
    game.scoring.proposals[user.id] = [...(game.scoring.proposals[opponent] || [])];
    game.scoring.confirmations[user.id] = nowIso(); game.scoring.confirmations[opponent] ||= nowIso();
    return this.confirmScoring(user, id);
  }

  resumeFromScoring(user, id) {
    const game = this.gameFor(id, user);
    if (game.status !== 'scoring' || !this.isParticipant(game, user)) throw new GoRuleError('当前不能恢复对局', 'INVALID_STATE');
    game.status = 'active'; game.scoring = null; game.events.push({ type: 'scoring-resumed', by: user.id, at: nowIso() });
    this.startTurn(game, this.position(game).toPlay); game.revision++; game.updatedAt = nowIso();
    this.save({ type: 'go-game-updated', gameId: game.id }, game.participantIds);
    return game;
  }
  ...
  markPresence(userId, connected) {
    let changed = false;
    for (const game of this.state.goGames.filter(item => ['active', 'scoring'].includes(item.status) && item.participantIds.includes(userId))) {
      game.disconnects ||= {};
