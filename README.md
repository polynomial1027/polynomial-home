# Polynomial Server 0.2.0 Demo

Polynomial Server 是一个单体 Node.js 门户，包含账号、聊天、网盘、网络学习、JupyterLab、小游戏与管理后台。0.2.0 新增完整的围棋中心和全站统一问题反馈。
...
- 终局处理：双方各自标记死子并确认；方案一致后服务器自动计算胜负，分歧时由双方调整、接受对方方案或恢复对局，管理员不参与实时裁决。
...
- KataGo 仅为人机模式所需；未安装时，研究、题库、联机和共享棋盘仍可使用
