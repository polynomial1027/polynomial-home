# Polynomial Server Home 0.1.5

完整门户包含：首页、项目、小游戏、网盘、账号系统、实时聊天，以及集课程、代码实验、随机作业评测和按账号隔离的 JupyterLab 于一体的网络学习中心。

0.1.5 在第一章最前面新增“名称、引用与内存身份”前置课。课程以教材形式讲解对象的身份、类型和值，`==` 与 `is`、CPython 的 `-5～256` 小整数缓存、常量复用、不可变对象的重新绑定、可变对象别名、函数的对象共享传递、字符串驻留和 `id()` 的生命周期限制。同时修复聊天输入栏缩窄、消息区无限增高和长文本越过页脚的问题：输入区恢复全宽，聊天记录在固定区域内滚动，长文本、文件名和附件会限制在消息卡片内；发起聊天窗口的关闭按钮不再触发成员校验，并新增私聊与聊天组区别说明。课程现有 11 章、65 个课节、140 个可独立运行示例和 15 道作业，并同步提供专业英文版本。已有进度、草稿、消息和提交记录保持兼容。
