# Polynomial Server Home 0.12.3

完整门户包含：首页、项目、小游戏、网盘、账号系统、实时聊天，以及集课程、代码实验、随机作业评测和按账号隔离的 JupyterLab 于一体的网络学习中心。

0.12.3 采用测试阶段版本规则：0 表示尚在测试，12 表示当前功能版本，最后一位表示小修改次数。本版将全部课程作业统一为 `class Solution` 方法格式，为每题加入调用与返回示例，并新增查看其他用户主动公开的成绩及答案代码功能。此前的统一页脚、独立作业流程、课程、容器令牌修复、后台布局、网盘布局和中文文件名修复均完整保留。
