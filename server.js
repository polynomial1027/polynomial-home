...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', raw => {
    try {
      ...
      if (!text) return;
      const message = { id: crypto.randomUUID(), userId: user.id, username: user.username, displayName: user.displayName, text, createdAt: new Date().toISOString() };
      state.messages.push(message); state.messages = state.messages.slice(-1000); save(state);
      const payload = JSON.stringify({ type: 'message', message }); wss.clients.forEach(client => { if (client.readyState === 1) client.send(payload); });
    } catch {}
  });
});
