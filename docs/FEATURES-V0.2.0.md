...
正式对局在双方连续停一手后进入数目阶段。双方分别标记死子并提交方案；方案一致时服务器按本局规则和贴目自动计算结果。方案不一致时，只能由双方继续修改、接受对方方案或恢复下棋，不存在“请求管理员裁决”流程。
...
- 未使用 KataGo 自动判定双方有争议的死活；这是有意设计，避免把管理员变成实时裁判，也避免 Demo 阶段由 AI 强制裁决。
